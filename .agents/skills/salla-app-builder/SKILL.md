...
first install** — e.g. collecting credentials before the app activates, or configuring
...
- **Yes** → use `salla_onboarding_steps` (one `action`-driven tool): `action=create` each
  ...
  `Resp.error().setFields(...)` (stop + show feedback). **Full tool params, the `update`
  revalidation rule, the Step Function handler, and the completion payload shape:
