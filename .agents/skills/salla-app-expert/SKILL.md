...
| Twilight themes                       | `salla_themes` · reads `list` `get` `github_config` `categories` `components` `settings` `screenshots` `publishing`; writes `create` `update_details` `screenshot_set` `component_create` `component_update` `settings_update` `publish` `set_status` and the preview/sample store actions — create needs a GitHub App installation; component and settings writes commit to the theme repo; see [salla-theme-builder](../salla-theme-builder/SKILL.md)                             |
