...
| Twilight themes                       | `salla_themes` · reads `list` `get` `github_config` `categories` `components` `component_get` `settings` `screenshots` `publishing` `preview_stores` `sample_stores`; writes `create` `update_details` `screenshot_set` `screenshot_delete` `component_create` `component_update` `component_delete` `settings_update` `publish` `publish_withdraw` `set_status` `preview_store_set` `preview_store_delete` `default_preview_store` `sample_store_set` `sample_store_delete` — create needs a GitHub App installation; component and settings writes commit to the theme repo; see [salla-theme-builder](../salla-theme-builder/SKILL.md) |
