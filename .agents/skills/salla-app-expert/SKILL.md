...
| Publish an app — validate + save a draft, then partner submits in the Portal                       | [salla-publication-consistency](../salla-publication-consistency/SKILL.md) |
...
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Create / configure / publish apps     | `salla_apps` · `list` `get` `create` `update` `connect` (OAuth+webhooks) `set_status` `publish_private` (private apps); public apps publish via `app_publish` (below)                                                                                                                                               |
...
| Publish an app (stepwise)             | Public: `app_publish` · `open` `set` `readiness` `validate` — `validate` saves a DRAFT (no admin submit); the partner then submits one-click in the Portal `/publish` page. Private: `salla_apps action=publish_private` (one shot). See [salla-publication-consistency](../salla-publication-consistency/SKILL.md) |
