...
| Publish an app (stepwise)             | `app_publish` · `open` `set` `readiness` `validate` — `validate` saves a DRAFT (no admin submit); the partner then submits one-click in the Portal `/publish` page; bulk `salla_apps action=publish` still saves a draft one-shot — see [salla-publication-consistency](../salla-publication-consistency/SKILL.md) |
