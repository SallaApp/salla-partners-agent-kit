...
6. **Validate + save the draft** (`action=validate`). On **422**, `set` the missing sections
   ...
   review. **Auto-check:** `salla_events action=list` shows the **subscribable** subscription
   events subscribed — `app.subscription.started` (+ `app.trial.*` when there's a trial) — **and**
   `salla_apps action=get` shows a `webhook_url` set. Only `started`/`app.trial.*` are
   subscribable; `app.subscription.renewed`/`expired`/`canceled` are NOT separate catalog entries —
   the platform delivers them to the `webhook_url` automatically once a subscription exists, so they
   are covered in the **handler**, not by a subscribe call (→ **salla-app-billing** Step 2).
   This only proves the events are wired at the **platform** level — **not** that the handler code
   is deployed and working. So also **verify a real round-trip:** fire a test subscription event on
   a demo store and confirm the app provisions/revokes correctly (→ **salla-live-testing**).
...
| "I subscribed to `app.subscription.started`, that's enough to go to review."      | Subscribing ≠ handling. `started` (+ `app.trial.*`) is all that's subscribable; the platform auto-delivers `renewed`/`expired`/`canceled` to the `webhook_url` once a subscription exists. The auto-check proves the events are wired at the PLATFORM level, not that the handler is deployed and working. The app must provision on `started`, re-point on `renewed`, revoke on `expired`/`canceled` (check `item_type`), call the renew API for `external_recurring`, and be verified by a real test event (salla-live-testing) before submit (Step 6b). |
| "I'll subscribe to `app.subscription.renewed`/`expired`/`canceled` too."          | They aren't in the subscribable catalog — only `app.subscription.started` (+ `app.trial.*`) is. The platform delivers renewed/expired/canceled to your `webhook_url` automatically once a subscription exists; cover them in the HANDLER, not via subscribe (Step 6b → salla-app-billing Step 2).                                                                                                                                                                                                                                                          |
