...
| Addons                         | `publication.addons`                                           | all types, max 5; `{name,description,price,slug,support_renew}` — `name`, `price`, `slug` **required** on every entry; prices in **SAR**; always one-time; `support_renew:true` ⇒ `external_recurring`. |
