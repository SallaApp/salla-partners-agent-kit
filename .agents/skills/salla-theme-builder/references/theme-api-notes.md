...
| `sample_store_*`    | `SampleStoreRequest::prepareForValidation` overwrites `name` and `url` from the theme's active-store lookup and nulls `store_id` when that store isn't active, so **`store_id` alone identifies a sample store**. Capped at 6 per theme. Behind a Portal feature flag, like `default_preview_store` | Pass `store_id` and `image_id` (800×450); anything you send for name/url is discarded. An update merges the current row before replacing it. A 404 means the company lacks the feature, not that the theme is missing |
