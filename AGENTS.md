...
- `hooks/` — a **SessionStart hook** (Claude Code auto-discovers `hooks/hooks.json`;
  ...
  Plus an **optional PostToolUse telemetry hook** (`hooks/track-telemetry`, registered on
  Claude Code / Codex / Cursor) that records a metadata-only `skill_invocation` event —
  **disabled by default — enable by setting `SALLA_TELEMETRY_URL`, disable by unsetting it**,
  and never sends the prompt, tool args, or any content. See `hooks/README.md`.
