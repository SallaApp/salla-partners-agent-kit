...
- **Red Flags tables** on the gated skills most exposed to rationalization shortcuts
  ...
  pairs a tempting "skip" thought with why it breaks in production.
...
- Reworded skill `description` frontmatter that lacked an explicit trigger so each
  ...
  (Cursor, Copilot).
- Moved heavy reference material (full code, payload shapes, error tables) out of the
  largest `SKILL.md` files into their `references/` sidecars to keep the entry point lean.
...
- Initial release: 25 skills covering the full Salla Partner app lifecycle (create, hook
