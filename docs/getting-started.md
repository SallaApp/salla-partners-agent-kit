...
| `salla_themes`           | `list` `get` `github_config` `categories` `create` · `update_details` · `screenshots` `screenshot_set` `screenshot_delete` · `components` `component_get` `component_create` `component_update` `component_delete` · `settings` `settings_update` · `publishing` `publish` `publish_withdraw` `set_status` · `preview_stores` `preview_store_set` `preview_store_delete` `default_preview_store` · `sample_stores` `sample_store_set` `sample_store_delete` | Twilight themes end to end: find, create, edit, components and settings, publish |
