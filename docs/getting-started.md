...
| `salla_themes`           | `list` `get` `github_config` `categories` `create` · `update_details` `screenshot_*` · `component_*` `settings_update` · `publish` `set_status` `preview_store_*` | Twilight themes end to end: find, create, edit, components and settings, publish |
